...
export function TopBar({ path, sourceState, canCopy, waitForReview, finishing, saving, openNotes, onCopy, onFinish, onCancel }: TopBarProps) {
  ...
  return (
    <header className="sticky top-0 z-40 grid h-10 grid-cols-[auto_minmax(0,1fr)_auto] items-center border-b bg-card px-2">
      ...
      <div className="truncate px-2 text-center text-xs font-medium" title={path} aria-label={path ? `Current document: ${path}` : undefined}>
      ...
      <div className="flex min-w-0 items-center justify-end gap-1">
        ...
        <ThemeToggle />
        <Button type="button" size="icon-sm" variant="ghost" aria-label="Copy feedback" title="Copy feedback" onClick={onCopy} disabled={!canCopy || saving || finishing}>
          <Copy />
        </Button>
        {waitForReview ? (
          <>
            <Button type="button" size="icon-sm" variant="ghost" aria-label="Cancel review" title="Cancel review" onClick={onCancel} disabled={finishing || saving}>
              <XCircle />
            </Button>
