...
test("a rejected stale save reloads server state and allows the next save", async () => {
  ...
  expect(screen.getByRole("spinbutton", { name: "From line" })).toHaveValue(1)
  expect(screen.getByRole("spinbutton", { name: "To line" })).toHaveValue(1)
  expect(screen.queryByText("Selected excerpt")).not.toBeInTheDocument()
