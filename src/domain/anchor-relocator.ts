...
function lineAt(offsets: number[], lines: number[], offset: number): number | null {
  let low = 0;
...
function uniqueNormalized(index: RelocateIndex, needle: string): AnchorMatch {
  ...
  while (from <= index.normalized.length) {
    ...
    if (offset < 0) break;
    const lineStart = lineAt(index.normalizedOffsets, index.normalizedLines, offset);
    const lineEnd = lineAt(index.normalizedOffsets, index.normalizedLines, offset + collapsed.length - 1);
...
function sameCluster(document: ReviewDocument, left: AnchorRange, right: AnchorRange): boolean {
  if (right.lineStart > left.lineEnd + 8) return false;
  for (let line = left.lineEnd + 1; line < right.lineStart; line += 1) {
    if (document.lines[line - 1]?.kind === "heading") return false;
  }
  return true;
}
